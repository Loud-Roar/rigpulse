...
when the firmware's TCP cgminer interface returns stale values.

See `RELEASE-NOTES-v0.6.3.md` for details.

## v0.6.2

Share totals and live share events now remain accurate when miner counters reset,
and session share counting is correctly initialized after a RigPulse restart.

See `RELEASE-NOTES-v0.6.2.md` for details.

## v0.6.1

Wallet balances now refresh every six hours, survive restarts, and remain visible
from the last successful update when a public balance service is unavailable.

See `RELEASE-NOTES-v0.6.1.md` for details.

## v0.6.0

This release introduces compact clickable BTC, BCH, and ALPH market cards with
custom icons, dedicated coin detail views, and Alephium wallet tracking.

See `RELEASE-NOTES-v0.6.0.md` for details.
...
hashrate, temperature, and power history charts.

See `RELEASE-NOTES-v0.5.12.md` for details.

## v0.5.11

This corrective release adds visible, editable topic fields for every ntfy sound
channel and prevents the Block Found badge from covering the miner name.

See `RELEASE-NOTES-v0.5.11.md` for details.
