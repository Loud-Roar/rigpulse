...
app = FastAPI(title="RigPulse", version="0.5.2")
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(10.0, connect=4.0)) as client:
                response = await client.get(
                    ...
                    headers={"accept": "application/json", "user-agent": "RigPulse/0.5.2"},
                )
...
body.theme-nerd-console .toolbar{margin-top:14px;background:#07131d;border-top:3px solid #d5ff43;padding:8px}body.theme-nerd-console .grid{gap:10px}body.theme-nerd-console .miner{color:#07131d;background:linear-gradient(145deg,#e5edf0 0 18%,#bfd5df 18% 72%,#8eb8c8 72%);border:3px solid #eefaff;border-top:7px solid #d5ff43;box-shadow:inset 0 0 0 3px #334d58,0 8px 20px #000b;clip-path:polygon(13px 0,calc(100% - 8px) 0,100% 8px,100% calc(100% - 13px),calc(100% - 13px) 100%,8px 100%,0 calc(100% - 8px),0 13px)}body.theme-nerd-console .miner.offline{border-color:#ffe8eb;border-top-color:#ff496a}body.theme-nerd-console .miner h3{color:#07131d;text-shadow:none;font-size:22px}body.theme-nerd-console .miner-head .sub,body.theme-nerd-console .miner .sub{color:#365764;font-weight:800}body.theme-nerd-console .miner .status{background:#07131d;color:#eaf8ff!important;border:2px solid #07131d}body.theme-nerd-console .miner .hash{display:block;color:#07120b;background:linear-gradient(180deg,#ddff54,#8edb2d);-webkit-background-clip:border-box;background-clip:border-box;-webkit-text-fill-color:#07120b;text-shadow:none;border:2px solid #496c25;padding:10px 12px;margin:11px 0 5px;text-align:right;font-size:clamp(30px,3vw,43px)}body.theme-nerd-console .miner .spark{background:#050b10;border:3px solid #3d5965}body.theme-nerd-console .miner .stats{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px}body.theme-nerd-console .miner .stats>div{position:relative;background:#dbe8ed;border:1px solid #71909d;border-left:6px solid #25cfe9;padding:8px 5px 8px 29px;min-height:48px}body.theme-nerd-console .miner .stats>div:nth-child(1){background:#70cda9;border-left-color:#20a776}body.theme-nerd-console .miner .stats>div:nth-child(2){background:#bdd9ed;border-left-color:#317bc0}body.theme-nerd-console .miner .stats>div:nth-child(4),body.theme-nerd-console .miner .stats>.current-share-stat{background:#f4e36e;border-left-color:#d39d00}body.theme-nerd-console .miner .stats span{color:#284754;font-weight:900}body.theme-nerd-console .miner .stats b{color:#07131d;font-size:14px}body.theme-nerd-console .console-stat-icon{display:grid;position:absolute;left:5px;top:8px;width:18px;height:18px;border-radius:50%;place-items:center;background:#07131d;color:#eaf8ff;font:900 11px Arial,sans-serif}body.theme-nerd-console .miner>div:last-child small{color:#264553!important}body.theme-nerd-console .miner.fleet-best{clip-path:none}
...
async function renderPoolMiners(){$('miners').innerHTML='<div class="sub">Loading SoloPool workers…</div>';try{const d=await fetch('/api/chain-status').then(r=>r.json()),sp=d.solopool||{},workers=[];if(filter!=='pool')return;for(const coin of ['btc','bch'])for(const w of(sp[coin]?.workers||[]))workers.push({...w,coin});workers.sort((a,b)=>String(a.name).localeCompare(String(b.name)));$('miners').innerHTML=workers.map(w=>{const on=!w.offline&&!w.dead;return `<article class="card miner pool-miner-card ${w.coin} ${on?'online':'offline'}"><div class="miner-head"><div><h3>${esc(w.name)}</h3><div class="sub">${w.coin.toUpperCase()} SoloPool · ${esc(w.geo||'Unknown region')}</div></div><div class="status ${on?'green':'red'}">● ${on?'Online':'Offline'}</div></div><div class="hash">${hashFromHs(w.hr)}</div><div class="sub">Current hashrate · Average: ${hashFromHs(w.avgHr)} · Last share: ${blockAge(w.lastShare)}</div><div class="spark">${spark()}</div><div class="stats"><div><span>Valid Shares</span><b>${Number(w.validShares||0).toLocaleString()}</b></div><div><span>Invalid</span><b>${Number(w.invalidShares||0).toLocaleString()}</b></div><div><span>Stale</span><b>${Number(w.staleShares||0).toLocaleString()}</b></div><div><span>Port</span><b>${w.port??'--'}</b></div><div><span>Region</span><b>${esc(w.geo||'--')}</b></div><div><span>Coin</span><b>${w.coin.toUpperCase()}</b></div></div></article>`}).join('')||'<div class="sub">No SoloPool workers found. Add a BTC or BCH SoloPool address in Customization.</div>';$('onlineCount').textContent=`${workers.filter(w=>!w.offline&&!w.dead).length} / ${workers.length} pool miners online`;loadFleetSummary();refreshHealth()}catch(e){if(filter==='pool')$('miners').innerHTML=`<div class="sub">Could not load SoloPool workers: ${esc(String(e))}</div>`}}
...
function renderSoloPool(coin,p,configured){const el=$(coin+'SoloPoolStatus');if(!p){el.innerHTML=`<div class="solopool-hash">—</div><div class="solopool-stats"><span>${configured?'Waiting for API…':'Not configured'}</span></div>`;return}const online=Number(p.online_workers||0),total=Number(p.total_workers||0),pct=total?Math.max(0,Math.min(100,online/total*100)):0;el.innerHTML=`<div class="solopool-console-main"><div class="pool-gauge" style="--pool-pct:${pct}%"><span>${online}/${total}<small>online</small></span></div><div><div class="solopool-hash">${hashFromHs(p.hashrate)}</div><div class="solopool-stats"><span>Current hashrate</span><span>Average <b>${hashFromHs(p.average_hashrate)}</b></span><span>Last share <b>${blockAge(p.last_share)}</b></span></div></div></div><div class="solopool-highlights"><div class="solopool-highlight"><span>Best Share</span><b>${fmtShare(p.best_share)}</b></div><div class="solopool-highlight"><span>Blocks Found</span><b>${p.total_blocks??0}</b></div></div>`}
