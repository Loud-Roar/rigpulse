...
app = FastAPI(title="RigPulse", version="0.7.1")
...
async def send_ntfy(title: str, message: str, *, priority: int = 4, tags: list[str] | None = None, channel: str = "best", require_enabled: bool = True) -> bool:
        ...
        try:
            ...
            for miner in miners:
                ...
                if old_accepted is not None and t.accepted is not None and t.accepted > old_accepted:
                    ...
                    last_local_share_at[miner["id"]] = ts
                    if ts - last_pool_fallback_at.get(miner["id"], 0) > 90:
                        record_event(miner["id"], "accepted_share", value_num=delta, details={"accepted": t.accepted, "algorithm": miner["algorithm"], "source": "miner"})
                ...
                if old:
                    ...
                    if old.online and not t.online:
                        warning = (f"RigPulse: {miner['name']} Offline", "The miner stopped responding.", "red_circle")
                    elif old.pool_alive is not False and t.pool_alive is False and alerts.get("pool_disconnect", True):
                        warning = (f"RigPulse: {miner['name']} Pool Disconnected", "The miner reports that its pool is not alive.", "warning")
...
def miners_get():
    with db() as c:
        ...
        for m in miners:
            ...
            pool_worker = matching_solopool_worker(m, rich)
            m["pool_shares_6h"] = int(pool_worker.get("validShares") or 0) if pool_worker else None
            m["pool_last_share"] = pool_worker.get("lastShare") if pool_worker else None
...
@app.post("/api/notifications/ntfy/test/{channel}")
async def ntfy_test(channel: str):
    cfg = get_customization()
    if not cfg.get("ntfy_enabled"):
        raise HTTPException(400, "Enable ntfy notifications first")
    if channel not in {"best", "block", "warning"}:
        raise HTTPException(400, "Unknown notification channel")
    try:
        titles = {"best":"RigPulse Best Share test", "block":"RigPulse BLOCK FOUND test", "warning":"RigPulse Warning test"}
        priorities = {"best":4, "block":5, "warning":4}
        tags = {"best":["trophy","pick"], "block":["tada","pick"], "warning":["warning","pick"]}
        sent = await send_ntfy(titles[channel], f"The RigPulse {channel} notification channel is connected.", priority=priorities[channel], tags=tags[channel], channel=channel)
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(10.0, connect=4.0)) as client:
                response = await client.get(
                    ...
                    headers={"accept": "application/json", "user-agent": "RigPulse/0.7.1"},
                )
...
def matching_solopool_worker(miner: dict[str, Any] | sqlite3.Row, telemetry: Telemetry | None) -> dict[str, Any] | None:
    url, user = _telemetry_pool_identity(telemetry)
    url_lower = url.lower()
    coin = "bch" if "bch" in url_lower else "btc" if "btc" in url_lower else None
    if not coin:
        return None
    worker_name = user.rsplit(".", 1)[-1] if "." in user else str(miner["name"])
    workers = (latest_solopool.get(coin) or {}).get("workers") or []
    return next((worker for worker in workers if str(worker.get("name", "")).lower() == worker_name.lower()), None)
                ...
                for coin, coin_addresses in addresses.items():
                    for address in coin_addresses:
                        try:
                            ...
                            now = int(time.time())
                            for worker_name, worker_data in (data.get("workers") or {}).items():
                                if not isinstance(worker_data, dict):
                                    continue
                                current_valid = int(worker_data.get("validShares") or 0)
                                counter_key = f"{coin}:{str(worker_name).lower()}"
                                previous_valid = latest_pool_worker_counts.get(counter_key)
                                latest_pool_worker_counts[counter_key] = current_valid
                                if previous_valid is None or current_valid <= previous_valid:
                                    continue
                                winner = None
                                for candidate in miner_rows:
                                    if candidate["family"] != "avalon":
                                        continue
                                    candidate_t = latest_telemetry.get(candidate["id"])
                                    candidate_url, candidate_user = _telemetry_pool_identity(candidate_t)
                                    candidate_worker = candidate_user.rsplit(".", 1)[-1] if "." in candidate_user else str(candidate["name"])
                                    candidate_coin = "bch" if "bch" in candidate_url.lower() else "btc" if "btc" in candidate_url.lower() else None
                                    accepted_source = str(((candidate_t.raw or {}) if candidate_t else {}).get("accepted_source") or "")
                                    if candidate_coin == coin and candidate_worker.lower() == str(worker_name).lower() and accepted_source == "cgminer TCP":
                                        winner = candidate
                                        break
                                if winner is None or now - last_local_share_at.get(winner["id"], 0) <= 90:
                                    continue
                                delta = min(current_valid - previous_valid, 25)
                                last_pool_fallback_at[winner["id"]] = now
                                details = {"source": "solopool_fallback", "coin": coin.upper(), "worker": worker_name, "valid_shares_6h": current_valid, "algorithm": winner["algorithm"]}
                                record_event(winner["id"], "accepted_share", value_num=delta, details=details)
                                await manager.broadcast({"type": "share", "miner_id": winner["id"], "miner_name": winner["name"], "algorithm": winner["algorithm"], "count": delta, "source": "solopool_fallback"})
...
<style>
@font-face{font-family:"RigRajdhani";src:url("https://raw.githubusercontent.com/google/fonts/main/ofl/rajdhani/Rajdhani-SemiBold.ttf") format("truetype");font-display:swap}@font-face{font-family:"RigRajdhani";src:url("https://raw.githubusercontent.com/google/fonts/main/ofl/rajdhani/Rajdhani-Bold.ttf") format("truetype");font-weight:800;font-display:swap}@font-face{font-family:"RigMono";src:url("https://raw.githubusercontent.com/google/fonts/main/ofl/sharetechmono/ShareTechMono-Regular.ttf") format("truetype");font-display:swap}
:root{
...
.spark{position:relative}.spark-tooltip{display:none;position:absolute;z-index:8;top:0;pointer-events:none;white-space:nowrap;background:rgba(4,13,24,.97);border:1px solid #38b6ff;border-radius:7px;color:#e5f7ff;font-size:10px;padding:4px 7px;box-shadow:0 5px 15px #0009;transform:translateX(-50%)}.spark.inspecting .spark-tooltip{display:block}.spark-marker{stroke:#b9edff;stroke-width:1;opacity:0}.spark.inspecting .spark-marker{opacity:1}
...
/* Nerd Console instrument face: typography, LCD hierarchy, iconography and gauges. */
body.theme-nerd-console{--nerd-lcd:#b8f34a;--nerd-lcd-hi:#d8ff76;--nerd-white:#f1f4e9;--nerd-muted:#91a48d;--nerd-line:#314339;--nerd-panel:#080c09;font-family:"RigRajdhani","Bahnschrift SemiCondensed","Arial Narrow",sans-serif}body.theme-nerd-console .brand,body.theme-nerd-console .top h1,body.theme-nerd-console .miner h3{font-family:"RigRajdhani","Arial Narrow",sans-serif;font-weight:800}body.theme-nerd-console .metric .value,body.theme-nerd-console .miner .hash,body.theme-nerd-console .miner .stats b,body.theme-nerd-console .solopool-hash,body.theme-nerd-console .solopool-highlight b,body.theme-nerd-console .wallet-item b,body.theme-nerd-console .block-height{font-family:"RigMono",Consolas,monospace}
body.theme-nerd-console .card{border-color:var(--nerd-line);box-shadow:inset 0 0 0 1px #111a13,0 10px 25px #000d}body.theme-nerd-console .miner{padding:0;background:linear-gradient(145deg,#090d0a 0 72%,#101a12 72%);border:1px solid #34483a;border-top:4px solid var(--nerd-lcd);clip-path:polygon(9px 0,calc(100% - 7px) 0,100% 7px,100% calc(100% - 9px),calc(100% - 9px) 100%,7px 100%,0 calc(100% - 7px),0 9px)}body.theme-nerd-console .miner-head{padding:11px 12px 7px;border-bottom:1px solid #26352b;background:linear-gradient(105deg,#0e150f,#050806)}body.theme-nerd-console .miner h3{font-style:normal;font-size:24px;line-height:.9;letter-spacing:.055em;color:var(--nerd-white)}body.theme-nerd-console .miner-head .sub{font-size:10px;color:#9caf99;letter-spacing:.08em}body.theme-nerd-console .miner .status{font-family:"RigMono",monospace;background:#070b08;border:1px solid #49634f;color:#c9efad!important;padding:5px 8px;box-shadow:inset 0 0 8px #000}
body.theme-nerd-console .miner .hash{margin:0 10px 5px;padding:11px 12px 8px;font-size:clamp(31px,2.45vw,42px);line-height:1;background:linear-gradient(180deg,var(--nerd-lcd-hi),#8dd72e);border:2px solid #506c27;border-radius:1px;box-shadow:inset 0 0 0 2px #c8ff6a55,inset 0 -8px 18px #559d1d55,0 0 12px #9bff3d22;letter-spacing:.01em}body.theme-nerd-console .miner>.sub{margin:3px 11px;color:#99aa96;font-size:11px}body.theme-nerd-console .miner .spark{margin:8px 10px;width:calc(100% - 20px);background:#020403;border:1px solid #35463b;box-shadow:inset 0 0 12px #000}body.theme-nerd-console .miner .spark svg polyline{stroke:#a7e64b!important}
body.theme-nerd-console .miner .stats{margin:0 10px;padding:7px 0 0;border-top:1px solid #26352b;gap:5px}body.theme-nerd-console .miner .stats>div,body.theme-nerd-console .miner .stats>div:nth-child(n){min-height:49px;padding:7px 5px 6px 35px;background:#080d09;border:1px solid #29382e;border-left:2px solid #617565}body.theme-nerd-console .miner .stats>div.stat-temp,body.theme-nerd-console .miner .stats>div.stat-fan{min-height:58px;padding-left:43px;background:linear-gradient(135deg,#0c160d,#070b08);border-color:#405445}body.theme-nerd-console .miner .stats>div.stat-share{border-left-color:#b7e64a}body.theme-nerd-console .miner .stats>div.stat-power{border-left-color:#d4e5bd}body.theme-nerd-console .miner .stats span{font-family:"RigRajdhani","Arial Narrow",sans-serif;color:var(--nerd-muted);font-size:9px;line-height:1;letter-spacing:.07em}body.theme-nerd-console .miner .stats b{color:var(--nerd-white);font-size:15px;line-height:1.1;margin-top:5px}
body.theme-nerd-console .console-stat-icon{left:7px;top:9px;width:22px;height:22px;background:#111b13;color:#b8d5b2;border:1px solid #3c5141;border-radius:3px}body.theme-nerd-console .console-stat-icon svg{width:15px;height:15px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}body.theme-nerd-console .stat-temp .console-stat-icon,body.theme-nerd-console .stat-fan .console-stat-icon{left:6px;top:10px;width:30px;height:30px;border-radius:50%;color:var(--nerd-lcd-hi);border:0;background:conic-gradient(var(--nerd-lcd) var(--gauge-pct,0%),#243028 0);box-shadow:0 0 8px #9cff3630}body.theme-nerd-console .stat-temp .console-stat-icon::before,body.theme-nerd-console .stat-fan .console-stat-icon::before{content:"";position:absolute;inset:4px;border-radius:50%;background:#080c09}body.theme-nerd-console .stat-temp .console-stat-icon svg,body.theme-nerd-console .stat-fan .console-stat-icon svg{position:relative;z-index:1;width:14px;height:14px}
body.theme-nerd-console .miner>div:last-child{margin:10px!important;padding:8px 0 1px;border-top:1px solid #26352b}body.theme-nerd-console .miner>div:last-child small{color:#849681!important;font-family:"RigMono",monospace}body.theme-nerd-console .miner .btn{background:#0a100b;border-color:#34483a;color:#dbe6d7}body.theme-nerd-console .miner .btn:hover{border-color:var(--nerd-lcd);color:var(--nerd-lcd-hi)}
body.theme-nerd-console .metric{border-top-color:#607165!important;background:linear-gradient(160deg,#060906 0 58%,#101712 58%)}body.theme-nerd-console .metric .label{color:#aab7a6}body.theme-nerd-console .metric .label::before{background:#111a13;color:#cbd7c7;border:1px solid #38483d}body.theme-nerd-console .metric .value,body.theme-nerd-console .metric:nth-child(n) .value{color:var(--nerd-lcd-hi);text-shadow:0 0 8px #a0ff3e35}body.theme-nerd-console .wallet-strip,body.theme-nerd-console .solopool-strip,body.theme-nerd-console .block-strip,body.theme-nerd-console .health-banner,body.theme-nerd-console .stream{border-color:var(--nerd-line)}body.theme-nerd-console .wallet-strip{border-left:5px solid var(--nerd-lcd)}body.theme-nerd-console .solopool-strip{border-left:5px solid var(--nerd-lcd);background:linear-gradient(110deg,#111811 0 14%,#050806 14%)}body.theme-nerd-console .solopool-highlight{border-color:#33463a;background:#080d09}body.theme-nerd-console .block-meta span{background:#0b100c;border-left-color:#82957f;color:#a7b2a4}
...
/* RigPulse Console: approved contemporary type and dark-green live hashrate instrument. */
body.theme-nerd-console{--nerd-lcd:#baff78;--nerd-lcd-hi:#dcffae;--nerd-white:#f0f6f1;--nerd-muted:#91a49a;--nerd-line:#294137;--nerd-panel:#07100b;font-family:"Aptos","Inter","Segoe UI",sans-serif;letter-spacing:0}body.theme-nerd-console .brand,body.theme-nerd-console .top h1,body.theme-nerd-console .miner h3,body.theme-nerd-console .metric .value,body.theme-nerd-console .miner .hash,body.theme-nerd-console .miner .stats b,body.theme-nerd-console .solopool-hash,body.theme-nerd-console .solopool-highlight b,body.theme-nerd-console .wallet-item b,body.theme-nerd-console .block-height{font-family:"Aptos Display","Aptos","Inter","Segoe UI",sans-serif}body.theme-nerd-console .brand,body.theme-nerd-console .top h1,body.theme-nerd-console .miner h3{font-style:normal;letter-spacing:-.025em;text-transform:none;text-shadow:none}
body.theme-nerd-console .miner{overflow:hidden;border:1px solid #294137;border-top:1px solid #3f6652;border-radius:20px;clip-path:none;background:radial-gradient(circle at 85% 8%,#17352455,transparent 34%),linear-gradient(145deg,#0b1510,#050a07 65%,#0c1710);box-shadow:inset 0 1px #dfffe810,0 18px 36px #0009}body.theme-nerd-console .miner-head{padding:15px 16px 11px;border-bottom:0;background:transparent}body.theme-nerd-console .miner h3{font-size:22px;line-height:1;color:#f0f6f1}body.theme-nerd-console .miner-head .sub{margin-top:5px;font-size:10px;letter-spacing:.05em;color:#82988b}body.theme-nerd-console .miner .status{font-family:"Aptos","Inter",sans-serif;border:0;border-radius:999px;background:#183023;color:#aef58a!important;padding:6px 9px;box-shadow:none;font-size:9px;letter-spacing:.04em}
body.theme-nerd-console .miner .hash{position:relative;display:flex;align-items:flex-end;justify-content:space-between;min-height:94px;margin:0 12px 7px;padding:33px 16px 18px;color:#baff78!important;-webkit-text-fill-color:#baff78;background:radial-gradient(circle at 84% 8%,#b2ff6a28,transparent 33%),linear-gradient(135deg,#285f358f,#091d11 58%,#06100a);border:0;border-radius:18px;box-shadow:inset 0 1px #e7ffdf22,inset 0 -1px #88ff5340,0 15px 30px #2d9a431f;font-size:clamp(31px,2.6vw,44px);font-weight:800;letter-spacing:-.055em;text-align:left;text-shadow:0 0 20px #97f25b30}body.theme-nerd-console .miner .hash::before{content:"LIVE HASHRATE";position:absolute;left:16px;top:13px;color:#8fb397;font-size:9px;font-weight:700;letter-spacing:.14em}body.theme-nerd-console .miner .hash::after{content:"";position:absolute;left:16px;right:16px;bottom:9px;height:2px;background:linear-gradient(90deg,#a7f56b,#a7f56b10);box-shadow:0 0 12px #a7f56b66}body.theme-nerd-console .miner>.sub{margin:4px 15px;color:#82988b;font-size:11px}
body.theme-nerd-console .miner .spark{margin:10px 13px;width:calc(100% - 26px);border:0;border-radius:12px;background:#040906;box-shadow:inset 0 0 0 1px #263c31}body.theme-nerd-console .miner .spark svg polyline{stroke:#8fec62!important}body.theme-nerd-console .miner .stats{margin:0 13px;padding:8px 0 0;border-top:1px solid #22342a;gap:6px}body.theme-nerd-console .miner .stats>div,body.theme-nerd-console .miner .stats>div:nth-child(n){min-height:48px;padding:8px 6px 7px 34px;border:0;border-radius:11px;background:#0b1510}body.theme-nerd-console .miner .stats>div.stat-temp,body.theme-nerd-console .miner .stats>div.stat-fan{background:linear-gradient(135deg,#102117,#09120d)}body.theme-nerd-console .miner .stats span{font-family:"Aptos","Inter",sans-serif;color:#7f9687;font-size:9px;letter-spacing:.045em}body.theme-nerd-console .miner .stats b{color:#eaf2eb;font-size:14px;font-weight:700;letter-spacing:-.015em}body.theme-nerd-console .console-stat-icon{border:0;border-radius:50%;background:#15291c;color:#a7e78a}
body.theme-nerd-console .metric,body.theme-nerd-console .block-strip,body.theme-nerd-console .wallet-strip,body.theme-nerd-console .solopool-strip,body.theme-nerd-console .health-banner,body.theme-nerd-console .stream{border:1px solid #294137;border-radius:18px;clip-path:none;background:linear-gradient(145deg,#0b1510,#050a07);box-shadow:inset 0 1px #e2ffe810,0 12px 28px #0007}body.theme-nerd-console .metric .label,body.theme-nerd-console .metric .sub,body.theme-nerd-console .block-meta,body.theme-nerd-console .solo-odds{font-family:"Aptos","Inter",sans-serif;text-transform:none;letter-spacing:0}body.theme-nerd-console .metric .value,body.theme-nerd-console .metric:nth-child(n) .value{color:#baff78;text-shadow:0 0 14px #90ee5830;font-weight:700;letter-spacing:-.035em}
@media(max-width:720px){body.theme-nerd-console .miner{border-radius:15px}body.theme-nerd-console .miner .hash{min-height:78px;margin:0 8px 6px;padding:29px 12px 15px;border-radius:14px}body.theme-nerd-console .miner .hash::before{left:12px;top:10px}body.theme-nerd-console .miner .hash::after{left:12px;right:12px;bottom:7px}body.theme-nerd-console .miner .spark,body.theme-nerd-console .miner .stats{margin-left:9px;margin-right:9px;width:calc(100% - 18px)}}
/* RigPulse integrated instrument layout. */
body.theme-nerd-console .grid{grid-template-columns:repeat(auto-fit,minmax(590px,1fr));gap:14px;align-items:start}body.theme-nerd-console .rig-console-miner{min-height:0;padding:0;border-radius:26px;background:radial-gradient(circle at 16% 8%,#2e6c3c2b,transparent 30%),linear-gradient(145deg,#0b1510,#040906 64%,#0b1510)}body.theme-nerd-console .rig-console-top{display:flex;align-items:center;justify-content:space-between;padding:15px 18px 12px;border-bottom:1px solid #21352a}body.theme-nerd-console .rig-console-brand{display:block;margin-bottom:4px;color:#a7f56b;font-size:9px;font-weight:800;letter-spacing:.18em}body.theme-nerd-console .rig-console-top h3{margin:0;font-size:24px}body.theme-nerd-console .rig-console-top .sub{margin-top:4px;font-size:10px;color:#809488}
body.theme-nerd-console .rig-console-face{display:grid;grid-template-columns:minmax(0,1.25fr) 112px minmax(190px,.9fr);grid-template-rows:185px 125px;min-height:310px}body.theme-nerd-console .rig-console-identity{grid-column:1;grid-row:1;padding:23px 20px;position:relative;background:radial-gradient(circle at 30% 25%,#42a46220,transparent 45%)}body.theme-nerd-console .rig-console-kicker{color:#82a08d;font-size:9px;font-weight:800;letter-spacing:.16em}body.theme-nerd-console .rig-console-uptime{margin-top:12px;color:#dce9df;font-size:18px;font-weight:700;letter-spacing:-.025em}body.theme-nerd-console .rig-console-share{position:absolute;left:20px;right:20px;bottom:19px;display:flex;align-items:flex-end;justify-content:space-between;padding-top:10px;border-top:1px solid #2a4735}body.theme-nerd-console .rig-console-share span{color:#7e9988;font-size:9px;font-weight:800;letter-spacing:.12em}body.theme-nerd-console .rig-console-share b{color:#dfffd0;font-size:24px;font-weight:800;letter-spacing:-.04em}
body.theme-nerd-console .rig-console-thermals{grid-column:2;grid-row:1/3;margin:12px 0;border-radius:24px;overflow:hidden;color:#112a1f;background:linear-gradient(180deg,#d9f5e5,#70cdb4);box-shadow:inset 0 1px #ffffff88,0 16px 30px #26765d29}body.theme-nerd-console .rig-console-gauge{height:50%;display:grid;place-items:center;align-content:center;gap:2px;position:relative;border-bottom:1px solid #1b5f4a33}body.theme-nerd-console .rig-console-gauge:last-child{border-bottom:0}body.theme-nerd-console .rig-console-gauge .console-stat-icon{position:relative;left:auto;top:auto;width:38px;height:38px;border:0;border-radius:50%;color:#17392c;background:conic-gradient(#245f47 var(--gauge-pct),#b7e9d7 0)}body.theme-nerd-console .rig-console-gauge .console-stat-icon::before{content:"";position:absolute;inset:4px;border-radius:50%;background:#c8eee0}body.theme-nerd-console .rig-console-gauge .console-stat-icon svg{position:relative;z-index:1;width:19px;height:19px}body.theme-nerd-console .rig-console-gauge b{font-size:20px;font-weight:800;letter-spacing:-.04em}body.theme-nerd-console .rig-console-gauge span{font-size:8px;font-weight:800;letter-spacing:.12em}
body.theme-nerd-console .rig-console-electrical{grid-column:3;grid-row:1;padding:20px 18px;display:grid;grid-template-columns:1fr 1fr;align-content:center;gap:18px 12px}body.theme-nerd-console .rig-console-electrical div{min-width:0}body.theme-nerd-console .rig-console-electrical span{display:block;color:#789083;font-size:8px;font-weight:800;letter-spacing:.09em}body.theme-nerd-console .rig-console-electrical b{display:block;margin-top:5px;color:#eef6f0;font-size:16px;font-weight:750;letter-spacing:-.025em;white-space:nowrap}
body.theme-nerd-console .rig-console-live{grid-column:1;grid-row:2;display:grid;grid-template-columns:minmax(0,1.5fr) minmax(112px,.72fr);gap:8px;margin:7px 11px 13px 14px}body.theme-nerd-console .rig-console-live .hash{min-height:0;height:100%;margin:0;padding:35px 15px 16px;border-radius:19px;font-size:clamp(29px,2.4vw,43px)}body.theme-nerd-console .rig-console-best-live{position:relative;display:flex;flex-direction:column;justify-content:flex-end;padding:15px 13px 18px;overflow:hidden;border-radius:19px;background:radial-gradient(circle at 70% 10%,#dcff7e29,transparent 43%),linear-gradient(145deg,#1b3420,#09130c);box-shadow:inset 0 1px #e8ffd71f,inset 0 -1px #baff7840}body.theme-nerd-console .rig-console-best-live::after{content:"";position:absolute;left:13px;right:13px;bottom:9px;height:2px;background:linear-gradient(90deg,#dfff62,#dfff6210);box-shadow:0 0 13px #caff5566}body.theme-nerd-console .rig-console-best-live span{position:absolute;top:13px;left:13px;color:#9ab58f;font-size:9px;font-weight:800;letter-spacing:.13em}body.theme-nerd-console .rig-console-best-live b{color:#e4ff9d;font-size:clamp(24px,2vw,34px);font-weight:850;letter-spacing:-.05em;text-shadow:0 0 15px #baff7840;white-space:nowrap}
body.theme-nerd-console .rig-console-history{grid-column:3;grid-row:2;margin:7px 14px 13px 10px;padding:11px 12px 8px;border-radius:18px;background:#050b07;box-shadow:inset 0 0 0 1px #243c2e}body.theme-nerd-console .rig-console-history-head{display:flex;justify-content:space-between;gap:8px;color:#789083;font-size:8px;font-weight:800;letter-spacing:.08em}body.theme-nerd-console .rig-console-history-head b{color:#b6c8ba;white-space:nowrap}body.theme-nerd-console .rig-console-history .spark{height:69px;width:100%;margin:7px 0 0;border:0;border-radius:0;background:transparent;box-shadow:none}
body.theme-nerd-console .rig-console-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 15px 13px;border-top:1px solid #21352a}body.theme-nerd-console .rig-console-footer>div:first-child{display:flex;gap:13px;flex-wrap:wrap;color:#7e9286;font-size:10px}body.theme-nerd-console .rig-console-footer .btn{padding:5px 8px;font-size:10px;border-radius:8px;clip-path:none}
@media(max-width:720px){body.theme-nerd-console .grid{grid-template-columns:1fr}body.theme-nerd-console .rig-console-face{grid-template-columns:minmax(0,1fr) 82px;grid-template-rows:145px 100px auto}body.theme-nerd-console .rig-console-identity{padding:14px 12px}body.theme-nerd-console .rig-console-share{left:12px;right:12px;bottom:11px}body.theme-nerd-console .rig-console-thermals{grid-column:2;grid-row:1/3;margin:6px 6px 6px 0;border-radius:15px}body.theme-nerd-console .rig-console-electrical{grid-column:1/3;grid-row:3;padding:12px;grid-template-columns:repeat(4,1fr);gap:8px}body.theme-nerd-console .rig-console-live{grid-column:1;grid-row:2;margin:5px 6px 7px;grid-template-columns:minmax(0,1.35fr) minmax(88px,.65fr);gap:5px}body.theme-nerd-console .rig-console-live .hash{margin:0;padding:29px 9px 12px;border-radius:13px;font-size:clamp(23px,7vw,34px)}body.theme-nerd-console .rig-console-best-live{padding:12px 8px 14px;border-radius:13px}body.theme-nerd-console .rig-console-best-live span{left:8px;top:9px;font-size:7px}body.theme-nerd-console .rig-console-best-live b{font-size:20px}body.theme-nerd-console .rig-console-history{display:none}body.theme-nerd-console .rig-console-footer{align-items:flex-start;flex-direction:column}}
/* v0.5.8 rollback: restore the proven v0.5.6 card density. */
body.theme-nerd-console .grid{grid-template-columns:repeat(auto-fit,minmax(390px,1fr));gap:10px}
...
<div id="detailHardware" style="display:none"></div><div id="detailCharts"><div class="sub" id="detailChartRange" style="margin:10px 2px 0">Hover or tap a graph for its exact timestamp and value.</div><div class="chart-wrap"><b>Hashrate history</b><canvas id="hashChart" class="chart-canvas"></canvas></div><div class="chart-wrap"><b>Temperature history</b><canvas id="tempChart" class="chart-canvas"></canvas></div><div class="chart-wrap"><b>Power history</b><canvas id="powerChart" class="chart-canvas"></canvas></div></div><div id="detailEventsWrap" style="display:none"><div class="section-title">Recent Events</div><div id="detailEvents"></div></div><div class="section-title">Troubleshooting</div><div class="tabs"><button class="btn" onclick="probe(selectedMinerId)">Probe API</button><button class="btn" onclick="diagnose(selectedMinerId)">Run Diagnostics</button><button class="btn" id="detailIceRaw" onclick="iceRaw(selectedMinerId)">IceRiver Raw</button></div></div></div>
...
function chartTime(ts,full=false){const d=new Date(Number(ts)*1000);if(!Number.isFinite(d.getTime()))return'--';return full?d.toLocaleString():d.toLocaleString([],{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}
function drawLineChart(id,rows,field,hoverIndex=null){
 const c=$(id);if(!c)return;const points=rows.filter(r=>r[field]!=null&&Number.isFinite(Number(r[field]))&&r.ts!=null);const dpr=window.devicePixelRatio||1,rect=c.getBoundingClientRect(),w=rect.width,h=rect.height;
 c.width=Math.max(300,Math.round(w*dpr));c.height=Math.max(160,Math.round(h*dpr));const x=c.getContext('2d');x.setTransform(dpr,0,0,dpr,0,0);x.clearRect(0,0,w,h);
 if(points.length<2){x.fillStyle='#8192a7';x.font='13px system-ui';x.fillText('Not enough data yet',12,24);return}
 const vals=points.map(r=>Number(r[field])),mn0=Math.min(...vals),mx0=Math.max(...vals),mn=mn0,mx=mx0===mn0?mn0+1:mx0,pL=38,pR=14,pT=18,pB=30,t0=Number(points[0].ts),t1=Number(points[points.length-1].ts),span=Math.max(1,t1-t0);
 const px=r=>pL+(w-pL-pR)*((Number(r.ts)-t0)/span),py=r=>h-pB-(Number(r[field])-mn)/(mx-mn)*(h-pT-pB);
 x.strokeStyle='#20334a';x.lineWidth=1;for(let i=0;i<4;i++){const y=pT+(h-pT-pB)*(i/3);x.beginPath();x.moveTo(pL,y);x.lineTo(w-pR,y);x.stroke()}
 x.strokeStyle='#38b6ff';x.lineWidth=2;x.beginPath();points.forEach((r,i)=>i?x.lineTo(px(r),py(r)):x.moveTo(px(r),py(r)));x.stroke();
 x.fillStyle='#8192a7';x.font='10px system-ui';x.textAlign='left';x.fillText(mx0.toFixed(1),2,pT+3);x.fillText(mn0.toFixed(1),2,h-pB+3);x.fillText(chartTime(t0),pL,h-7);x.textAlign='center';x.fillText(chartTime(t0+span/2),pL+(w-pL-pR)/2,h-7);x.textAlign='right';x.fillText(chartTime(t1),w-pR,h-7);
 if(hoverIndex!=null){const r=points[Math.max(0,Math.min(points.length-1,hoverIndex))],xx=px(r),yy=py(r),unit=field==='temp_c'?'°C':field==='power_w'?' W':'';x.strokeStyle='#b9edff';x.lineWidth=1;x.beginPath();x.moveTo(xx,pT);x.lineTo(xx,h-pB);x.stroke();x.fillStyle='#b9edff';x.beginPath();x.arc(xx,yy,4,0,Math.PI*2);x.fill();const label=`${chartTime(r.ts,true)}  •  ${Number(r[field]).toFixed(field==='hashrate'?2:1)}${unit}`;x.font='11px system-ui';const tw=x.measureText(label).width+16,bx=Math.max(4,Math.min(w-tw-4,xx-tw/2)),by=Math.max(3,yy-31);x.fillStyle='rgba(4,13,24,.96)';x.fillRect(bx,by,tw,23);x.strokeStyle='#38b6ff';x.strokeRect(bx,by,tw,23);x.fillStyle='#e5f7ff';x.textAlign='left';x.fillText(label,bx+8,by+15)}
 c._chart={rows,field,points};if(!c._chartBound){const inspect=e=>{const s=c._chart;if(!s?.points?.length)return;const box=c.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(e.clientX-box.left-38)/Math.max(1,box.width-52))),target=Number(s.points[0].ts)+ratio*(Number(s.points[s.points.length-1].ts)-Number(s.points[0].ts));let best=0;s.points.forEach((r,i)=>{if(Math.abs(Number(r.ts)-target)<Math.abs(Number(s.points[best].ts)-target))best=i});drawLineChart(c.id,s.rows,s.field,best)};c.addEventListener('pointermove',inspect);c.addEventListener('pointerdown',inspect);c.addEventListener('pointerleave',()=>{const s=c._chart;if(s)drawLineChart(c.id,s.rows,s.field)});c._chartBound=true}
}
...
function minerConsoleCard(m){const t=m.telemetry||{},on=!!t.online,status=on?'Online':t.reachable?'Reachable / No Telemetry':'Offline',statusTone=on?'green':t.reachable?'orange':'red',avg=t.avg_hashrate!=null?`${fmt(t.avg_hashrate)} ${t.avg_hashrate_unit||t.hashrate_unit||''}`:'--',temp=t.temp_c!=null?fmt(t.temp_c,0)+'°C':'--',fan=t.fan_rpm!=null?fmt(t.fan_rpm,0):'--',power=t.power_w!=null?fmt(t.power_w,0)+' W':'--',eff=t.efficiency!=null?fmt(t.efficiency,1)+' '+(t.efficiency_unit||''):'--',reject=m.reject_pct!=null?fmt(m.reject_pct,3)+'%':'--',pool=t.pool_alive===true?'Alive':t.pool_alive===false?'Down':'--',poolShares=m.pool_shares_6h!=null?` · Pool 6h ${Number(m.pool_shares_6h).toLocaleString()}`:'';return `<article class="card miner rig-console-miner ${on?'online':'offline'} clickable" onclick="openDetail(${m.id})"><header class="rig-console-top"><div><span class="rig-console-brand">RIGPULSE</span><h3>${esc(m.name)}</h3><div class="sub">${esc(m.model||m.family)} · ${esc(m.algorithm)}</div></div><div class="status ${statusTone}">● ${status}</div></header><div class="rig-console-face"><section class="rig-console-identity"><div class="rig-console-kicker">MINER STATUS</div><div class="rig-console-uptime">${t.uptime_s!=null?uptime(t.uptime_s):'--'} uptime</div><div class="rig-console-share current"><span>CURRENT SHARE</span><b>${fmtShare(t.current_share)}</b></div></section><section class="rig-console-thermals"><div class="rig-console-gauge" style="--gauge-pct:${Math.max(0,Math.min(100,Number(t.fan_rpm||0)/60))}%">${consoleIcon('fan')}<b>${fan}</b><span>RPM</span></div><div class="rig-console-gauge" style="--gauge-pct:${Math.max(0,Math.min(100,Number(t.temp_c||0)))}%">${consoleIcon('temp')}<b>${temp}</b><span>CHIP TEMP</span></div></section><section class="rig-console-electrical"><div><span>ϟ POWER</span><b>${power}</b></div><div><span>↗ EFFICIENCY</span><b>${eff}</b></div><div><span>◎ LIFETIME</span><b>${m.shares_lifetime??'--'}</b></div><div><span>× REJECT</span><b>${reject}</b></div></section><section class="rig-console-live"><div class="hash">${hashText(t,m.algorithm)}</div><div class="rig-console-best-live"><span>BEST SHARE</span><b>${fmtShare(t.best_share)}</b></div></section><section class="rig-console-history"><div class="rig-console-history-head"><span>1H HASHRATE</span><b>AVG ${avg}</b></div><div class="spark" id="spark-${m.id}"></div></section></div><footer class="rig-console-footer" onclick="event.stopPropagation()"><div><span>${esc(m.ip)}</span><span>Today ${m.shares_today??'--'} · Session ${m.shares_session??'--'}${poolShares}</span><span class="${t.pool_alive===true?'green':t.pool_alive===false?'red':''}">Pool ${pool}</span></div><div><button class="btn" onclick="editMiner(${m.id})">Edit</button> <button class="btn danger" onclick="deleteMiner(${m.id})">Delete</button></div></footer></article>`}
function renderConsolePreview(){if(filter==='pool'){renderPoolMiners();return}let shown=miners.filter(m=>filter==='all'||m.algorithm===filter);const ss={name:(a,b)=>String(a.name).localeCompare(String(b.name)),hashrate:(a,b)=>(b.telemetry?.hashrate??-1)-(a.telemetry?.hashrate??-1),temp:(a,b)=>(b.telemetry?.temp_c??-1)-(a.telemetry?.temp_c??-1),power:(a,b)=>(b.telemetry?.power_w??-1)-(a.telemetry?.power_w??-1),efficiency:(a,b)=>(a.telemetry?.efficiency??999999)-(b.telemetry?.efficiency??999999)};shown.sort(ss[sortBy]||ss.name);const online=miners.filter(m=>m.telemetry?.online).length;$('miners').innerHTML=shown.map(minerConsoleCard).join('')||'<div class="sub">No miners yet.</div>';$('onlineCount').textContent=`${online} / ${miners.length} online`;loadFleetSummary();refreshHealth();shown.forEach(m=>loadMiniSpark(m.id))}
function render(){if(filter==='pool'){renderPoolMiners();return}let shown=miners.filter(m=>filter==='all'||m.algorithm===filter);const ss={name:(a,b)=>String(a.name).localeCompare(String(b.name)),hashrate:(a,b)=>(b.telemetry?.hashrate??-1)-(a.telemetry?.hashrate??-1),temp:(a,b)=>(b.telemetry?.temp_c??-1)-(a.telemetry?.temp_c??-1),power:(a,b)=>(b.telemetry?.power_w??-1)-(a.telemetry?.power_w??-1),efficiency:(a,b)=>(a.telemetry?.efficiency??999999)-(b.telemetry?.efficiency??999999)};shown.sort(ss[sortBy]||ss.name);let out='',online=miners.filter(m=>m.telemetry?.online).length;for(const m of shown){let t=m.telemetry||{},on=!!t.online,poolShares=m.pool_shares_6h!=null?` · Pool 6h: ${Number(m.pool_shares_6h).toLocaleString()}`:'';out+=`<article class="card miner ${on?'online':'offline'} clickable" onclick="openDetail(${m.id})"><div class="miner-head"><div><h3>${esc(m.name)}</h3><div class="sub">${esc(m.model||m.family)} · ${esc(m.algorithm)}</div></div><div class="status ${on?'green':t.reachable?'orange':'red'}">● ${on?'Online':t.reachable?'Reachable / No Telemetry':'Offline'}</div></div><div class="hash">${hashText(t,m.algorithm)}</div>${t.avg_hashrate!=null?`<div class="sub">Avg: ${fmt(t.avg_hashrate)} ${t.avg_hashrate_unit||t.hashrate_unit||''}</div>`:''}<div class="sub">Today: ${m.shares_today??'--'} · Session: ${m.shares_session??'--'}${poolShares}</div><div class="spark" id="spark-${m.id}"></div><div class="stats"><div><span>Max Temp</span><b>${t.temp_c!=null?fmt(t.temp_c,0)+'°C':'--'}</b></div><div><span>Power</span><b>${t.power_w!=null?fmt(t.power_w,0)+' W':'--'}</b></div><div><span>Lifetime</span><b>${m.shares_lifetime??'--'}</b></div><div><span>Best</span><b>${fmtShare(t.best_share)}</b></div><div><span>Efficiency</span><b>${t.efficiency!=null?fmt(t.efficiency,1)+' '+(t.efficiency_unit||''):'--'}</b></div><div><span>Reject %</span><b>${m.reject_pct!=null?fmt(m.reject_pct,3)+'%':'--'}</b></div><div><span>Uptime</span><b>${t.uptime_s!=null?uptime(t.uptime_s):'--'}</b></div><div><span>Pool</span><b class="${t.pool_alive===true?'green':t.pool_alive===false?'red':''}">${t.pool_alive===true?'Alive':t.pool_alive===false?'Down':'--'}</b></div></div>${m.family==='iceriver'&&t.raw?`<div class="sub" style="margin-top:10px">Board sensors: ${Array.isArray(t.raw.inlet_temps)&&t.raw.inlet_temps.length?t.raw.inlet_temps.join('/')+'°C':'--'} / ${Array.isArray(t.raw.outlet_temps)&&t.raw.outlet_temps.length?t.raw.outlet_temps.join('/')+'°C':'--'} · HW errors: ${t.raw.hardware_errors??'--'}</div>`:''}<div style="display:flex;justify-content:space-between;margin-top:12px" onclick="event.stopPropagation()"><small style="color:#7f91a7">${esc(m.ip)}</small><div><button class="btn" style="padding:5px 8px;font-size:11px" onclick="probe(${m.id})">Probe</button> <button class="btn" style="padding:5px 8px;font-size:11px" onclick="diagnose(${m.id})">Diagnose</button> ${m.family==='iceriver'?`<button class="btn" style="padding:5px 8px;font-size:11px" onclick="iceRaw(${m.id})">IceRiver Raw</button>`:''} <button class="btn" style="padding:5px 8px;font-size:11px" onclick="editMiner(${m.id})">Edit</button> <button class="btn danger" style="padding:5px 8px;font-size:11px" onclick="deleteMiner(${m.id})">Delete</button></div></div></article>`}$('miners').innerHTML=out||'<div class="sub">No miners yet.</div>';$('onlineCount').textContent=`${online} / ${miners.length} online`;loadFleetSummary();refreshHealth();shown.forEach(m=>loadMiniSpark(m.id))}
...
async function loadMiniSpark(id){try{let rows=await fetch(`/api/miners/${id}/history?seconds=3600`).then(r=>r.json()),h=$(`spark-${id}`);if(!h)return;const samples=rows.filter(r=>r.hashrate!=null&&Number.isFinite(Number(r.hashrate))&&r.ts!=null),v=samples.map(r=>Number(r.hashrate));if(v.length<2){h.innerHTML='<div class="sub" style="padding:10px 0">Collecting history…</div>';return}let mn=Math.min(...v),mx=Math.max(...v);if(mx===mn)mx=mn+1;const t0=Number(samples[0].ts),t1=Number(samples[samples.length-1].ts),span=Math.max(1,t1-t0),pts=samples.map(r=>{const xx=(Number(r.ts)-t0)/span*195,yy=40-(Number(r.hashrate)-mn)/(mx-mn)*34;return`${xx.toFixed(1)},${yy.toFixed(1)}`}),m=miners.find(x=>x.id===id),unit=m?.telemetry?.hashrate_unit||'';h.innerHTML=`<svg viewBox="0 0 195 45" preserveAspectRatio="none" style="width:100%;height:42px"><polyline fill="none" stroke="#38b6ff" stroke-width="1.5" points="${pts.join(' ')}"/><line class="spark-marker" x1="0" x2="0" y1="2" y2="43"/></svg><div class="spark-tooltip"></div>`;const inspect=e=>{e.stopPropagation();const box=h.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(e.clientX-box.left)/Math.max(1,box.width))),target=t0+ratio*span;let best=0;samples.forEach((r,i)=>{if(Math.abs(Number(r.ts)-target)<Math.abs(Number(samples[best].ts)-target))best=i});const r=samples[best],pos=(Number(r.ts)-t0)/span*100,marker=h.querySelector('.spark-marker'),tip=h.querySelector('.spark-tooltip');marker.setAttribute('x1',String(pos*1.95));marker.setAttribute('x2',String(pos*1.95));tip.style.left=`${pos}%`;tip.textContent=`${chartTime(r.ts,true)} • ${Number(r.hashrate).toFixed(2)} ${unit}`;h.classList.add('inspecting')};h.addEventListener('pointermove',inspect);h.addEventListener('pointerdown',inspect);h.addEventListener('pointerleave',()=>h.classList.remove('inspecting'));h.addEventListener('click',e=>e.stopPropagation())}catch(e){}}
...
async function loadDetailHistory(seconds){if(!selectedMinerId)return;let rows=await fetch(`/api/miners/${selectedMinerId}/history?seconds=${seconds}`).then(r=>r.json());const timed=rows.filter(r=>r.ts!=null),range=$('detailChartRange');if(range)range.textContent=timed.length?`${chartTime(timed[0].ts,true)} through ${chartTime(timed[timed.length-1].ts,true)} · Hover or tap for an exact reading.`:'No timestamped samples in this period.';requestAnimationFrame(()=>{drawLineChart('hashChart',rows,'hashrate');drawLineChart('tempChart',rows,'temp_c');drawLineChart('powerChart',rows,'power_w')})}
...
async function loadChainExtras(){
 try{
  ...
  if(b.available){const difficulty=fmtDifficulty(b.difficulty);$('bchBlockHeight').textContent=`BCH Block ${Number(b.height).toLocaleString()}`;$('bchBlockHash').textContent=shortHash(b.hash);$('bchBlockAge').textContent='Live';$('bchBlockTx').textContent=b.transactions_24h==null?'—':Number(b.transactions_24h).toLocaleString();$('bchMempool').textContent=b.mempool_transactions==null?'—':Number(b.mempool_transactions).toLocaleString();$('bchDifficulty').textContent=difficulty;$('bchSummaryDifficulty').textContent=difficulty;$('bchDifficultyFreshness').textContent=`updated ${blockAge(b.updated_at)} ago`;$('bchBlockState').textContent='● LIVE';$('bchBlockState').className='status green'}
  else{$('bchBlockHeight').textContent='BCH unavailable';$('bchSummaryDifficulty').textContent='Unavailable';$('bchDifficultyFreshness').textContent='retrying';$('bchBlockState').textContent='● WAITING';$('bchBlockState').className='status orange'}
...
function consoleIcon(kind){const p={temp:'<path d="M9 3v8.2a4 4 0 1 0 6 0V3a3 3 0 0 0-6 0Z"/><path d="M12 6v8"/>',fan:'<circle cx="12" cy="12" r="2"/><path d="M12 10c-1-6 4-7 5-4 1 3-2 5-5 6M14 12c6-1 7 4 4 5-3 1-5-2-6-5M12 14c1 6-4 7-5 4-1-3 2-5 5-6M10 12c-6 1-7-4-4-5 3-1 5 2 6 5"/>',power:'<path d="m13 2-7 12h6l-1 8 7-12h-6Z"/>',share:'<circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="M12 2v3M22 12h-3M12 22v-3M2 12h3"/>',best:'<path d="M8 4h8v5a4 4 0 0 1-8 0Z"/><path d="M8 6H4v2a4 4 0 0 0 4 4M16 6h4v2a4 4 0 0 1-4 4M12 13v5M8 21h8M9 18h6"/>',uptime:'<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',pool:'<circle cx="12" cy="12" r="8"/><path d="M8 12h8M12 8v8"/>',reject:'<circle cx="12" cy="12" r="9"/><path d="m9 9 6 6M15 9l-6 6"/>',efficiency:'<path d="M4 16a8 8 0 0 1 16 0"/><path d="m12 16 5-5M6 19h12"/>',lifetime:'<path d="m12 3 8 4-8 4-8-4 8-4Z"/><path d="m4 12 8 4 8-4M4 17l8 4 8-4"/>',default:'<path d="M5 6h14M5 12h14M5 18h14"/>'};return `<i class="console-stat-icon"><svg viewBox="0 0 24 24" aria-hidden="true">${p[kind]||p.default}</svg></i>`}
function stabilizeMinerCards(){applyFleetBestRing();document.querySelectorAll('.miner').forEach(card=>{if(card.classList.contains('pool-miner-card'))return;const id=Number((card.getAttribute('onclick')||'').match(/openDetail\((\d+)\)/)?.[1]),m=miners.find(x=>x.id===id),spark=$(`spark-${id}`);if(spark&&sparkCache.has(id))spark.innerHTML=sparkCache.get(id);card.classList.toggle('block-winner',!!m?.block_found);if(m?.block_found&&!card.querySelector('.block-found-badge'))card.insertAdjacentHTML('afterbegin','<div class="block-found-badge">⛏ BLOCK FOUND</div>');if(!m?.block_found)card.querySelector('.block-found-badge')?.remove();const stats=card.querySelector('.stats');if(stats&&!stats.querySelector('.current-share-stat'))stats.children[3]?.insertAdjacentHTML('afterend',`<div class="current-share-stat"><span>Current Share Difficulty</span><b>${fmtShare(m?.telemetry?.current_share)}</b></div>`);if(stats&&!stats.querySelector('.fan-stat'))stats.insertAdjacentHTML('beforeend',`<div class="fan-stat"><span>Fan Speed</span><b>${m?.telemetry?.fan_rpm!=null?fmt(m.telemetry.fan_rpm,0)+' RPM':'--'}</b></div>`);if(stats)stats.querySelectorAll(':scope>div').forEach(cell=>{const label=(cell.querySelector('span')?.textContent||'').toLowerCase(),kind=label.includes('temp')?'temp':label.includes('power')?'power':label.includes('best')?'best':label.includes('current')?'share':label.includes('fan')?'fan':label.includes('uptime')?'uptime':label.includes('pool')?'pool':label.includes('reject')?'reject':label.includes('efficiency')?'efficiency':label.includes('lifetime')?'lifetime':'default';cell.classList.add(`stat-${kind}`);if(!cell.querySelector('.console-stat-icon'))cell.insertAdjacentHTML('afterbegin',consoleIcon(kind));if(kind==='temp')cell.style.setProperty('--gauge-pct',`${Math.max(0,Math.min(100,Number(m?.telemetry?.temp_c||0)))}%`);if(kind==='fan')cell.style.setProperty('--gauge-pct',`${Math.max(0,Math.min(100,Number(m?.telemetry?.fan_rpm||0)/60))}%`)})})}
